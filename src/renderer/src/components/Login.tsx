...
const Login: React.FC<LoginProps> = ({ providerList, onSuccess, onClose, cloud, handleCloudChoose }) => {
  ...
  const handleSignIn = async (id: string): Promise<void> => {
    ...
    try {
      ...
      console.error(err);
      setError((err instanceof Error ? err.message : String(err)) || "An error occurred during sign-in");
      setStatus("error");
    }
  };
  ...
  return (
    <div className="relative w-80 rounded-lg bg-white p-6 shadow-lg dark:bg-gray-800">
        ...
        {providerList.map(({ id, label, icon }) => {
          ...
          return (
            <li key={id}>
              ...
              >
                <img src={icon} alt={`${label} icon`} className="mr-2 h-6 w-6" />
                <span>{label}</span>
              </button>
            </li>
          );
      ...
      {status === "error" && <p className="mt-2 text-red-500">Error: {error}</p>}
    </div>
  );
};
