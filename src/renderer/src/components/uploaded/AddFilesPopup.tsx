    ...
    return (
      <li key={item.path} className="flex flex-col">
        ...
        >
          {isDir ? (
            <>
              <button onClick={() => toggleDir(item.path)} className="flex-shrink-0" aria-label="Toggle folder">
                {isExpanded ? (
          ...
          <p className="flex-1 text-sm leading-snug break-all" title={item.path}>
            {item.path.split(/[/\\]/).pop()}
          </p>
          ...
          {depth === 0 ? (
            ...
            >
              {isExcluded ? <Undo size={14} /> : <Trash size={14} />}
            </button>
          )}
        </div>
        ...
        {isDir && isExpanded && (
          <ul className="space-y-2">
            {loadingPath === item.path ? (
              <li className="pl-6 text-xs text-gray-400 italic">Loading...</li>
            ) : hasChild ? (
              children.map((child) => renderEntry(child, depth + 1))
            ) : (
              <li className="pl-6 text-xs text-gray-400 italic">Empty</li>
            )}
          </ul>
        )}
      </li>
          ...
          {selectedItems.length > 0 && (
            <>
              ...
              <p className="mb-4 text-right text-xs text-gray-500 dark:text-gray-400">
                Total size: <span className="font-medium">{formatBytes(totalSize)}</span>
              </p>
            </>
          )}

          {}
          <Button variant="outline" className="w-full justify-start" onClick={chooseFolder} disabled={!hasAccount}>
            <FolderIcon className="mr-2 h-4 w-4" /> Choose Folder
          </Button>

          {}
          <Button variant="outline" className="w-full justify-start" onClick={chooseFiles} disabled={!hasAccount}>
            ...
            <Select value={selected} onValueChange={setSelected}>
              ...
              <SelectContent>
                {hasAccount ? (
                  ...
                  <div className="p-3 text-sm text-gray-500 dark:text-gray-400">No cloud accounts connected.</div>
                )}
              </SelectContent>
            </Select>
          </div>
