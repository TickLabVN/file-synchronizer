...
export class CredentialStore<T = unknown> {
  ...
  async add(account: string, tokens: T): Promise<void> {
    await keytar.setPassword(this.serviceName, account, JSON.stringify(tokens));
  ...
  async list(): Promise<{ account: string; tokens: T }[]> {
    try {
      ...
      console.error(`[CredentialStore] keytar error (${this.serviceName})`, e);
    }
  ...
  private _pullCache(account: string): void {
    const arr = (store.get(this.cacheKey, []) as string[]).filter((a) => a !== account);
