...
import type { ICloudProvider, AuthAccount, SyncOptions, SyncResult, TrackedFiles } from "../lib/ICloudProvider";
...
import { UploadHooks, CompareHooks, DownloadHooks, CleanupHooks } from "../utils/types";
...
export default class BoxProvider implements ICloudProvider {
  ...
  private readonly credStore = new CredentialStore<BoxTokens>("com.filesynchronizer.box", "boxAccounts");
  ...
  async signIn(): Promise<AuthAccount> {
    ...
    return new Promise<AuthAccount>((resolve, reject) => {
      ...
      const handleRedirect = async (url: string): Promise<void> => {
        ...
        try {
          const { data: tokens } = await axios.get(`${BACKEND_URL}/auth/box/token?code=${code}`);
      ...
      authWin.webContents.on("will-navigate", (_, url) => handleRedirect(url));
      authWin.on("closed", () => {
        if (!handled) reject(new Error("[Box.auth] Authentication window was closed by user"));
      });
  ...
  async getProfile(id: string): Promise<AuthAccount> {
    const tokens = await this.credStore.get(id);
    if (!tokens) throw new Error(`No saved tokens for ${id}`);
    await axios.post(`${BACKEND_URL}/auth/box/set-tokens`, tokens).catch(() => {});
    if (tokens.name) {
  ...
  private async getBoxClient(): Promise<BoxClient> {
    if (!this.activeAccount) throw new Error("[Box.client] No active Box account selected");
    const stored = await this.credStore.get(this.activeAccount);
    if (!stored?.refresh_token) throw new Error(`[Box.client] No refresh token for ${this.activeAccount}`);

    const { data: fresh } = await axios.post(`${BACKEND_URL}/auth/box/refresh-tokens`, {
  ...
  private buildUploadHooks(box: BoxClient): UploadHooks {
    return {
      uploadFolder: async (name, parentId, localPath) => {
        ...
        try {
          await box.folders.addMetadata(folderId, "global", "properties", {
            originalPath: localPath,
            os: process.platform,
          });
        } catch (err: unknown) {
          if ((err as BoxError).response?.status !== 409) console.warn("[Box] folder metadata", err);
        }
        return folderId;
      },
      ...
      uploadFile: async (localPath, parentId) => {
        ...
        const uploaded = Array.isArray(uploadRes.entries) ? uploadRes.entries[0] : uploadRes;
        const fileId: string = uploaded.id;
        try {
          await box.files.addMetadata(fileId, "global", "properties", {
            originalPath: localPath,
            os: process.platform,
          });
        } catch (err: unknown) {
          if ((err as BoxError).response?.status !== 409) console.warn("[Box] file metadata", err);
        }
        return fileId;
      },

      updateFile: async (remoteId, localPath) => {
        await box.files.uploadNewFileVersion(remoteId, fs.createReadStream(localPath));
      },

      setMetadata: async (remoteId, meta) => {
        try {
          await box.files.updateMetadata(remoteId, "global", "properties", meta);
        } catch (err: unknown) {
          if ((err as BoxError).response?.status !== 409) console.warn("[Box] setMetadata", err);
        }
      },
  ...
  private buildCompareHooks(box: BoxClient): CompareHooks {
    return {
      ...
      deleteRemote: async (remoteId, isDir) => {
        if (isDir) await box.folders.delete(remoteId, { recursive: true });
    ...
    return {
      listChildren: async (parentId) => {
        ...
        while (true) {
          ...
          for (const item of entries) {
            ...
            try {
              if (item.type === "file" && item.name === LOCK_NAME) continue;
              if (item.type === "file") meta = await box.files.getMetadata(item.id, "global", "properties");
              else if (item.type === "folder") meta = await box.folders.getMetadata(item.id, "global", "properties");
            } catch (err: unknown) {
              if ((err as BoxError).response?.status !== 404) console.warn("[Box] metadata error", err);
            }
            children.push({
              id: item.id,
              name: item.name,
      ...
      readFile: async (remoteId, dest) => {
        await new Promise<void>((resolve, reject) => {
          box.files.getReadStream(remoteId, undefined, (err: unknown, stream: Readable) => {
            if (err) return reject(err);
            stream.pipe(dest).on("finish", resolve).on("error", reject);
          });
        });
      },
  ...
  async sync(options: SyncOptions): Promise<SyncResult> {
    ...
    const { acquired, lockId } = await this.acquireLock(backupFolderId, deviceId);
    if (!acquired) {
    ...
    try {
      ...
      for (const p of paths) {
        ...
        try {
          ...
          if (err && typeof err === "object" && "code" in err && (err as { code?: string }).code === "ENOENT") {
            console.warn(`[Box.sync] Path "${p}" missing locally, cleaning up on Box...`);
      ...
      if (lockId) {
        try {
          ...
          broadcast("app:toast", "Box sync failed to release lock on cloud");
        }
      }
    }
  }
  ...
  async autoSync(): Promise<boolean> {
    ...
    try {
      for (const key of mappingStore.keys()) {
        ...
        try {
          const changed = await traverseCompare(key, rec.id, compareHooks, stopSync);
          anyChanged ||= changed;
        } catch (err) {
  ...
  async pull(): Promise<boolean> {
    ...
    try {
      ...
      const roots = pickRootPaths(username, this.id).filter((p) => !p.startsWith(centralFolderPath + path.sep));
      ...
      if (lockId) {
        try {
          ...
          broadcast("app:toast", "Box pull failed to release lock on cloud");
        }
      }
    }
  }
  ...
  async deleteTrackedFile(src: string): Promise<boolean> {
    ...
    const box = await this.getBoxClient();
    try {
      if (rec.isDirectory) await box.folders.delete(rec.id, { recursive: true });
    ...
    const cfgPath = path.join(app.getPath("userData"), "central-config.json");
    const { centralFolderPath } = JSON.parse(await fs.promises.readFile(cfgPath, "utf-8"));
    const linkPath = path.join(centralFolderPath, path.basename(src));
    try {
      await fs.promises.unlink(linkPath);
    } catch {
      console.warn("[Box.deleteTrackedFile] unlink symlink failed", linkPath);
    }
  ...
  async acquireLock(
    ...
    const existing = entries.find((e: BoxLockEntry) => e.type === "file" && e.name === LOCK_NAME);
    ...
    if (existing) {
      ...
      try {
        ...
        if (status !== 404 && status !== 403) {
          console.warn("[Box.acquireLock] Failed to delete existing lock:", existing.id, (err as Error).message);
        }
      }
    }

    const res = await box.files.uploadFile(backupFolderId, LOCK_NAME, Buffer.from(""));
  ...
  async releaseLock(lockId: string): Promise<void> {
    ...
    if (lockId) {
      try {
        ...
        const status = (err as BoxError).response?.status;
        if (status === 404) {
          console.debug(`[Box.releaseLock] Lock ${lockId} already deleted`);
        } else if (status === 403) {
          console.warn(`[Box.releaseLock] Lock ${lockId} could not be deleted (403 Forbidden)`);
        } else {
          console.debug(`[Box.releaseLock] Lock ${lockId} already gone (status ${status})`);
        }
      }
    }
  }
  ...
  async cleanupLockOnExit(): Promise<void> {
    ...
    const lock = entries.find((e: BoxLockEntry) => e.type === "file" && e.name === LOCK_NAME);
    if (lock) {
      try {
        await box.files.delete(lock.id);
        console.log("[Box.exit] Deleted lock file on Box");
      } catch (err) {
        console.warn("[Box.exit] Failed to delete lock file on Box:", err);
      }
    }
  }
  ...
  private async ensureBackupFolder(box: BoxClient): Promise<string> {
    ...
    const existing = entries.find((it: BoxFolderEntry) => it.type === "folder" && it.name === folderName);
