...
export default class GoogleDriveProvider implements ICloudProvider {
  ...
  private readonly credStore = new CredentialStore<GoogleDriveTokens>("com.filesynchronizer.googledrive", "gdAccounts");
    ...
    return new Promise<AuthAccount>((resolve, reject) => {
      ...
      const handleRedirect = async (url: string): Promise<void> => {
        if (url.startsWith("myapp://oauth")) {
          ...
          try {
            const { data: tokens } = await axios.get(`${BACKEND_URL}/auth/google/token?code=${code}`);
      ...
      authWin.on("closed", () => {
        if (!handled) {
          reject(new Error("[Drive.auth] Authentication window was closed by user"));
        }
      });
      ...
      if (!t.name) {
        try {
          const payload = JSON.parse(
            Buffer.from((tokens as GoogleDriveTokens).id_token.split(".")[1], "base64").toString()
          );
  ...
  private async getDriveClient(): Promise<drive_v3.Drive> {
    ...
    if (!stored) throw new Error("[Drive.client] Tokens not found for " + this.activeAccount);

    await axios.post(`${BACKEND_URL}/auth/google/set-tokens`, stored);

    const { data: fresh } = await axios.get(`${BACKEND_URL}/auth/google/refresh-tokens`);
  ...
  private buildDownloadHooks(drive: drive_v3.Drive): DownloadHooks {
    return {
      ...
      readFile: async (remoteId, dest) => {
        const res = await drive.files.get({ fileId: remoteId, alt: "media" }, { responseType: "stream" });
  ...
  async sync(options: SyncOptions): Promise<SyncResult> {
    ...
    try {
      ...
      const driveUsername = user?.displayName || user?.emailAddress || "Unknown";
      ...
      for (const p of paths) {
        ...
        try {
          ...
          if (err && typeof err === "object" && "code" in err && (err as { code?: string }).code === "ENOENT") {
            console.warn(`[Drive.sync] Path "${p}" missing locally, cleaning up on Drive...`);
      ...
      if (lockId) {
        try {
          ...
          broadcast("app:toast", "Google Drive sync failed to release lock on cloud");
        }
      }
    }
  }
  ...
  async autoSync(): Promise<boolean> {
    ...
    if (!acquired) {
      ...
      broadcast("app:toast", "Google Drive auto-sync skipped, lock already held");
      return true;
    }
    ...
    try {
      ...
      if (lockId) {
        try {
          ...
          broadcast("app:toast", "Google Drive auto-sync failed to release lock on cloud");
        }
      }
    }
  }
    ...
    try {
      ...
      if (lockId) {
        try {
          ...
          broadcast("app:toast", "Google Drive pull failed to release lock on cloud");
        }
      }
    }
  }
    ...
    if (lockId) {
      try {
        ...
        if (status !== 404 && status !== 403) {
          ...
          console.debug(`[Drive.releaseLock] Lock ${lockId} already gone (status ${status})`);
        }
      }
    }
  }
    ...
    if (data.files && data.files.length) {
      ...
      if (typeof fileId === "string") {
        try {
          ...
          console.warn("[Drive.exit] Failed to delete lock file on Google Drive:", err);
        }
      }
    }
  }
   ...
   */
  private async folderExists(drive: drive_v3.Drive, id: string): Promise<boolean> {
    try {
