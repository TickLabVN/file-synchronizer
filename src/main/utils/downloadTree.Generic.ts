...
export default async function downloadTree(
  ...
  for (const child of children) {
    ...
    const destPath: string = meta.originalPath && isSameOS ? meta.originalPath : path.join(localDir, child.name);
    ...
    if (isDirectory) {
      await downloadTree(child.id, destPath, hooks, { provider, account }, entries);
      continue;
    }
